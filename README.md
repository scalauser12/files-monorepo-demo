...
The build keeps the standard monorepo lifecycle intact and customizes it through the grouped policy settings rather than a raw process override. On `sbt-release-io-monorepo` `0.13.7`, these grouped `.sbt` keys remain the supported configuration surface for this build:
